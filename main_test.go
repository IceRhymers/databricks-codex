...
func TestParseArgs_HelpLong(t *testing.T) {
	verbose, version, showHelp, printEnv, noOtel, _, _, _, _, upstream, logFile, profile, otel, codexArgs := parseArgs([]string{"--help"})
	if !showHelp {
...
func TestParseArgs_LogFile(t *testing.T) {
	_, _, _, _, _, _, _, _, _, _, logFile, _, _, _ := parseArgs([]string{"--log-file", "/tmp/test.log"})
...
func TestParseArgs_Upstream(t *testing.T) {
	_, _, _, _, _, _, _, _, _, upstream, _, _, _, _ := parseArgs([]string{"--upstream", "https://gw.example.com/openai/v1"})
...
func TestParseArgs_NoOtel(t *testing.T) {
	_, _, _, _, noOtel, _, _, _, _, _, _, _, _, codexArgs := parseArgs([]string{"--no-otel"})
	if !noOtel {
...
func TestParseArgs_OtelTable(t *testing.T) {
	_, _, _, _, _, otelMetricsTable, otelMetricsTableSet, _, _, _, _, _, _, _ := parseArgs([]string{"--otel-table", "main.custom.metrics"})
	if !otelMetricsTableSet {
		t.Error("expected otelMetricsTableSet=true when --otel-table is passed")
	}
	if otelMetricsTable != "main.custom.metrics" {
		t.Errorf("expected otelMetricsTable=%q, got %q", "main.custom.metrics", otelMetricsTable)
	}
}
...
func TestParseArgs_OtelTableEquals(t *testing.T) {
	_, _, _, _, _, otelMetricsTable, otelMetricsTableSet, _, _, _, _, _, _, _ := parseArgs([]string{"--otel-table=main.custom.metrics"})
	if !otelMetricsTableSet {
		t.Error("expected otelMetricsTableSet=true for --otel-table=value")
	}
	if otelMetricsTable != "main.custom.metrics" {
		t.Errorf("expected otelMetricsTable=%q, got %q", "main.custom.metrics", otelMetricsTable)
	}
...
func TestParseArgs_OtelTableDefault(t *testing.T) {
	_, _, _, _, _, otelMetricsTable, otelMetricsTableSet, _, _, _, _, _, _, _ := parseArgs([]string{})
	if otelMetricsTableSet {
		t.Error("expected otelMetricsTableSet=false when --otel-table is not passed")
	}
	if otelMetricsTable != "main.codex_telemetry.codex_otel_metrics" {
		t.Errorf("expected default otelMetricsTable, got %q", otelMetricsTable)
	}
}

func TestParseArgs_OtelMetricsTable(t *testing.T) {
	_, _, _, _, _, otelMetricsTable, otelMetricsTableSet, _, _, _, _, _, _, _ := parseArgs([]string{"--otel-metrics-table", "main.custom.metrics"})
	if !otelMetricsTableSet {
...
func TestParseArgs_OtelLogsTable(t *testing.T) {
	_, _, _, _, _, _, _, otelLogsTable, otelLogsTableSet, _, _, _, _, _ := parseArgs([]string{"--otel-logs-table", "main.custom.logs"})
	if !otelLogsTableSet {
...
func TestParseArgs_UnknownFlagPassthrough(t *testing.T) {
	_, _, _, _, _, _, _, _, _, _, _, _, _, codexArgs := parseArgs([]string{"--unknown"})
...
func TestParseArgs_EmptyArgs(t *testing.T) {
	verbose, version, showHelp, printEnv, noOtel, _, _, _, _, upstream, logFile, profile, otel, codexArgs := parseArgs([]string{})
...
func TestParseArgs_Mixed(t *testing.T) {
	verbose, _, showHelp, _, _, _, _, _, _, upstream, _, _, _, _ := parseArgs([]string{"--verbose", "--upstream", "https://gw.example.com", "--help"})
	if !showHelp {
...
func TestParseArgs_Separator(t *testing.T) {
	verbose, _, _, _, _, _, _, _, _, _, _, _, _, codexArgs := parseArgs([]string{"--verbose", "--", "--unknown", "arg1"})
	if !verbose {
...
func TestParseArgs_PassthroughArgs(t *testing.T) {
	_, _, _, _, _, _, _, _, _, _, _, _, _, codexArgs := parseArgs([]string{"prompt text", "--model", "gpt-4"})
	if len(codexArgs) != 3 {
...
func TestParseArgs_Table(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			verbose, version, showHelp, printEnv, noOtel, _, _, _, _, upstream, logFile, profile, otel, codexArgs := parseArgs(tc.args)
...
func TestParseArgs_Profile(t *testing.T) {
	_, _, _, _, _, _, _, _, _, _, _, profile, _, _ := parseArgs([]string{"--profile", "aidev"})
	if profile != "aidev" {
...
func TestParseArgs_ProfileEquals(t *testing.T) {
	_, _, _, _, _, _, _, _, _, _, _, profile, _, _ := parseArgs([]string{"--profile=production"})
...
func TestParseArgs_Otel(t *testing.T) {
	_, _, _, _, _, _, _, _, _, _, _, _, otel, _ := parseArgs([]string{"--otel"})
	if !otel {
...
func TestHandleHelp_AllFlagsPresent(t *testing.T) {
	...
	flags := []string{"--profile", "--upstream", "--verbose", "-v", "--log-file", "--otel", "--no-otel", "--otel-metrics-table", "--otel-logs-table", "--otel-table", "--version", "--help"}
...
func TestDeriveLogsTable_StandardSuffix(t *testing.T) {
	got := deriveLogsTable("main.codex_telemetry.codex_otel_metrics")
...
func TestDeriveLogsTable_NoSuffix(t *testing.T) {
	got := deriveLogsTable("main.custom.table")
	want := "main.custom.table_otel_logs"
	if got != want {
		t.Errorf("got %q, want %q", got, want)
	}
}
