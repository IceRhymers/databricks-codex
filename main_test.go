...
func TestParseArgs_Table(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "--headless-release sets headlessRelease",
			args: []string{"--headless-release"},
			want: result{headlessRelease: true},
		},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			verbose, version, showHelp, printEnv, noOtel, _, _, upstream, logFile, profile, otel, _, _, _, model, modelSet, portFlag, headless, _, installHooks, uninstallHooks, headlessEnsure, headlessRelease, codexArgs := parseArgs(tc.args)
			...
			if headlessRelease != tc.want.headlessRelease {
				t.Errorf("headlessRelease: got %v, want %v", headlessRelease, tc.want.headlessRelease)
			}
...
func TestHandleHelp_AllFlagsPresent(t *testing.T) {
	...
	flags := []string{"--profile", "--model", "--upstream", "--verbose", "-v", "--log-file", "--otel", "--no-otel", "--otel-logs-table", "--port", "--headless", "--headless-ensure", "--headless-release", "--idle-timeout", "--install-hooks", "--uninstall-hooks", "--version", "--help"}
